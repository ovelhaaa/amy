...
| `chorus_mode` | 0 Classic, 1 Juno, 2 Ensemble, 3 Wide, 4 Vibrato | mapped to current (0 Off, 1..5) |
...
- Files written during the intermediate stabilization window have no reliable
  ...
  should re-save them (the writer always emits the current v6) from a known-good
  state.
